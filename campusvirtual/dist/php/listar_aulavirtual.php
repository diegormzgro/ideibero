...
$con = "SELECT * FROM catg_examen WHERE idAsignacion='id' AND Eliminado=0 AND Estatus =0";
...
AND u.id_grupo = d.idGrupo
    AND s.Eliminado=0 AND d.Estatus=1 AND u.id='$id' AND 
...
INNER JOIN usuario_activo AS u ON u.id_posgrado = a.idPosgrado 
INNER JOIN catg_posgrado AS p ON p.id= a.idPosgrado 
  WHERE d.idDocente='$id' AND a.Eliminado=0 AND s.Eliminado=0 AND d.Estatus=1  AND 
  DATE(s.FechaSesion)=CURDATE() 
