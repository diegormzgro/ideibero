...
if(($_SESSION['emailInst'])!=''){
    ...
    $sql= " SELECT a.Materia, s.NombreSesion , d.id, d.idGrupo
    FROM catg_asignatura as a
     INNER JOIN catg_asignacion_docente AS d ON a.idMateria = d.idAsignatura

 INNER JOIN catg_sesiones as s ON s.idMateria= d.id 
    WHERE d.id='$idMateria' AND s.Eliminado=0 AND a.Eliminado=0";
...
<center> <h1 class="mt-4">Alumnos</h1></center>
      ...
      $sql= "SELECT  u.id, u.ap_paterno, u.ap_materno, u.nombre, u.emailInst,  p.abrev AS Posgrado, pe.NombrePerfil AS Perfil, IF(u.bloqueado=1,'Bloqueado','Activo') AS estatus
            ...
            WHERE u.id_perfil<>0 AND d.id='$idMateria'  AND u.bloqueado=0 AND u.eliminado=0 AND u.emailInst<>'' AND d.idGrupo= u.id_grupo
            ORDER BY u.id_perfil,  u.ap_paterno,  u.ap_materno, u.nombre
            ";
            $datos = mysqli_query($conexion,$sql);
            mysqli_set_charset($conexion,"utf8");
            $contador=0;
            ...
            $sqlCali ="SELECT c.idAsignatura, c.idAlumno, c.idDocente, c.Docente, c.Alumno, c.Calificacion, c.idCalificacion FROM catg_calificaciones_examen AS c WHERE c.Eliminado=0 AND c.idDocente='$id' AND c.idAlumno='$idAlumno' AND  idAsignatura='$idMateria' ";
            ...
            echo '</td>
            <td>
              <a href="javascript:ver_examen('.$idAlumno.','.$idMateria.');"><i class="btn-outline-success far fa-eye" title="Trabajos"></i>
              </a>';
              if ($Calificacion!="") {
        echo '
                <a href="javascript:editarCalificacion('.$idAlumno.','.$idMateria.');">
              <img src="opciones/editar.png" title="Calificación">
              </a>';
              }else{
                echo '
                <a href="javascript:nueva_calificacion('.$idAlumno.','.$idMateria.');">
              <img src="opciones/editar.png" title="Calificación">
              </a>';
              }
...
function ver_examen(id, idMateria){
  $('#contenido').load("php/listar_examen.php?Asignacion="+idMateria, "idAlumno="+id);
  $("body").toggleClass("sb-sidenav-toggled");
