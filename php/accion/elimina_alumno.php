...
$sql = "SELECT * FROM usuario_activo WHERE id = '$id'";
    ...
    while ($renglon=mysqli_fetch_array($datos)) {
      $bloqueado = $renglon['bloqueado'];
      $idPosgrado = $renglon['id_posgrado'];
    }

if ($bloqueado==1) {
$sql = "UPDATE usuario_activo SET bloqueado = 0 WHERE id = '$id' ";
$registro = mysqli_query($conexion,$sql);
}else{
	$sql = "UPDATE usuario_activo SET bloqueado = 1 WHERE id = '$id' ";
$registro = mysqli_query($conexion,$sql);
}
...
WHERE u.id_perfil<>0 AND u.id_posgrado='$idPosgrado'
