...
if ($_FILES['ProgramaEstudio']['name']!=""){
	...
	if ($filas>0) {
	$sql = "UPDATE catg_asignacion_docente as a SET idDocente='$docente', ProgramaEstudio='$nombre', Estatus='$status' WHERE a.idAsignatura='$id' AND a.Eliminado=0 AND a.idGrupo='$idGrupo' ";
    $datos = mysqli_query($conexion,$sql);
	}else{
    $sql = "INSERT INTO catg_asignacion_docente (idGrupo, idAsignatura, idDocente, ProgramaEstudio, Estatus) "."VALUES ($idGrupo, $catedra, '$docente', '$nombre', $status)";
    $datos = mysqli_query($conexion,$sql);  
    }
